...
function editOrSave(button, fieldType) {
  if (button.classList.contains('Edit')) {
    ...
    saveTut(
        ...
        {[fieldType]: newFieldInput.value})  // dict with all props to change
        .then((newDict) => {  // update UI after saving
...
// Visual update: Replace field with new input and save button for all three fields
...
function saveTut(tutID, newDict) {
  ...
  keysToUpdate.forEach((key) => {  // check per key if valid prop (for Tutoring model)
    if (['date', 'duration', 'subject', 'teacher', 'student', 'content'].includes(key)) {
      // If the key is valid, add it to the updatePayload
      if (key === 'date' && newDict[key]) {  // Format the date as yyyy-mm-dd
